...
class CheckPoint(OpenMedicMonitorOpBase):
    def __init__(self, model_path: str, is_replace: bool=False, early_stopping: dict={}, save_best: dict={}):
    ...
    def initialize(cls,
        ...
        **kwargs
        ...
        if save_best:
            ...
            if patience == 0:
                raise OpenMedicMonitorOpError("Can not set `patience` equal to 0. If you do not want apply earl stopping, please remove `patience`.")
    ...
    def execute(self, model: OpenMedicModelBase, **kwargs):
        ...
        if self.save_best:
            ...
            assert isinstance(score, float), f"{score_name} can not be found as input parameters in `CheckPoint.execute()`"
            ...
            if self.best_score:
                ...
                if flag:
                    logging.info(f"[CheckPoint][execute]: Found best score {score} from {self.best_score}")
