...
class ModelRegister(BaseRegister):
    ...
    __MODULES: List[str] = ["unet"]

    @classmethod
    def import_modules(cls):
        for module in cls.__MODULES:
            module_interface: utils.ModuleInterface = utils.import_module(
                module_name=cls.__TEMPLATE.format(module=module),
            )
...
class TransformRegister(BaseRegister):
    ...
    __MODULES: List[str] = ["resize", "flip"]

    @classmethod
    def import_modules(cls):
        for module in cls.__MODULES:
            model_interface: utils.ModuleInterface = utils.import_module(
                module_name=cls.__TEMPLATE.format(module=module),
            )
...
class MetricRegister(BaseRegister):
    ...
    def register(
        cls,
        metric_class: metric.OpenMedicMetricOpBase,
        metric_name: str = "",
    ):
        if not metric_name:
...
class MonitorRegister(BaseRegister):
    ...
    def register(
        cls,
        monitor_class: monitor.OpenMedicMonitorOpBase,
        monitor_name: str = "",
    ):
        if not monitor_name:
